...
## Corpus Check
- 61 files · ~17,981 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 537 nodes · 1661 edges · 17 communities detected
- Extraction: 43% EXTRACTED · 57% INFERRED · 0% AMBIGUOUS · INFERRED: 943 edges (avg confidence: 0.62)
...
1. `RunLogger` - 60 edges
2. `AnnotatorResult` - 41 edges
3. `TestLinkageAnnotator` - 37 edges
4. `CoChangeAnnotator` - 36 edges
5. `DocstringAnnotator` - 35 edges
6. `RadonAnnotator` - 35 edges
7. `BanditAnnotator` - 33 edges
8. `CoverageAnnotator` - 33 edges
9. `PipAuditAnnotator` - 33 edges
10. `RuffAnnotator` - 33 edges
...
Cohesion: 0.06
Nodes (71): _finding_payload(), Bandit security annotator., _relative_to_repo(), _severity_num(), build_context(), build_node_analysis_prompt(), evidence_corpus(), _node_summary() (+63 more)

### Community 2 - "Community 2"
Cohesion: 0.07
Nodes (45): AnalysisSummary, analyze_top_n(), _attach_findings(), build_request(), _cache_key(), _finding_payload(), _grammar_version(), LLMAnalyzer (+37 more)

### Community 3 - "Community 3"
Cohesion: 0.21
Nodes (45): BanditAnnotator, BaseAnnotator, Mutate ``graph`` with this annotator's signal., Check that the underlying tool is installed and runnable., BaseAnnotator, _DryRunProvider, _NoopCache, Command-line interface for qualgraph. (+37 more)

### Community 4 - "Community 4"
Cohesion: 0.09
Nodes (45): _add_edge(), _aliased_import_parts(), build_graph(), _call_name(), _class_base_names(), _class_definition_nodes(), _collect_python_files(), Definition (+37 more)
...
Cohesion: 0.08
Nodes (36): Build a NetworkX code graph from Python source files.  The builder intentionally, _module_node_for_file(), Git co-change annotator., Enum, Git history annotator., _relative_to_target(), _target_prefix(), Pip-audit vulnerability annotator. (+28 more)

### Community 7 - "Community 7"
Cohesion: 0.08
Nodes (21): ABC, AnnotatorResult, Base interface for graph annotators., _has_tests(), Coverage.py annotator., _relative_to_repo(), Docstring coverage annotator., _strip_docstring_quotes() (+13 more)
...
- **Why does `RunLogger` connect `Community 3` to `Community 2`, `Community 5`, `Community 7`?**
  _High betweenness centrality (0.096) - this node is a cross-community bridge._
- **Why does `AnnotatorResult` connect `Community 7` to `Community 1`, `Community 3`, `Community 6`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `NodeType` connect `Community 6` to `Community 1`, `Community 2`, `Community 3`, `Community 4`, `Community 7`, `Community 10`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Are the 53 inferred relationships involving `RunLogger` (e.g. with `_DryRunProvider` and `_NoopCache`) actually correct?**
  _`RunLogger` has 53 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 34 inferred relationships involving `TestLinkageAnnotator` (e.g. with `_DryRunProvider` and `_NoopCache`) actually correct?**
  _`TestLinkageAnnotator` has 34 INFERRED edges - model-reasoned connections that need verification._
- **Are the 32 inferred relationships involving `CoChangeAnnotator` (e.g. with `_DryRunProvider` and `_NoopCache`) actually correct?**
  _`CoChangeAnnotator` has 32 INFERRED edges - model-reasoned connections that need verification._